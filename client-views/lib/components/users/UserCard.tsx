...
const TeamLink: FC<{ team: CachedTeamMeta | null }> = ({ team }) => {
    const baseStyles = `
        min-w-15 h-20
...
const UserCard: FC<UserCardProps> = ({ user: { name, lastSolve, score, type }, team, userImgHref }) => (
    <div
        className="
        ...
        <span className="text-xl mt-6 mb-8">{type === "normal" ? "Participant" : "Non-Participant"}</span>
        <TeamLink team={team} />       
    </div>
);
